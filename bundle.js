...
(function (global) {
  ...
  const getEndDateForRange = (dateStr, timeFrom, timeTo) => {
    const base = new Date(`${dateStr || getLocalDateStr()}T00:00:00`);
  ...
  async function loadDataFromGitHub(forceRemote = false) {
    try {
      ...
      if (response.ok) {
        ...
        if (!forceRemote && localUpdated && localUpdated > remoteUpdated) {
          console.log("📱 Using local data (newer than remote)");
          return {
