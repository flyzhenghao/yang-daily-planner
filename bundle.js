        ...
        return (
            <div className="modal-overlay">
                <div className="modal" style={{ maxWidth: "420px" }}>
                    ...
                    >
                        {global.formatDate(activity.date)} ·{" "}
...
(function(global) {
    ...
    function Activities({
        ...
        const compareByDateTime = (a, b) => {
            const dc = String(a.date || "").localeCompare(
                String(b.date || ""),
            );
            if (dc !== 0) return dc;
            return String(a.timeFrom || "").localeCompare(
                String(b.timeFrom || ""),
            );
        };
    ...
    function ActivityRow({
        ...
        return (
            <tr onClick={() => onEdit && onEdit(a)}>
                ...
                {!condensed && (
                    <td data-label="Date">{global.formatDate(a.date)}</td>
                )}
                <td data-label="Time">
                    {a.timeFrom} - {a.timeTo}
                </td>
        ...
        return (
            <div className="calendar-grid">
                ...
                {days.map((day) => {
                    ...
                    const isToday =
                        global.getLocalDateStr(new Date()) === dayStr;
...
(function(global) {
    ...
    function Settings({
        ...
        const [savedGitHubToken, setSavedGitHubToken] =
            useState(global.getGitHubToken());
        ...
        const startDeleteCategory = (cat) => {
            ...
            const fallback =
                categories.find(
                    (c) =>
                        c.id !== cat.id && global.isOtherCategoryName(c.name),
                )?.name ||
...
(function(global) {
    ...
    function App() {
        ...
        const generateDailySummary = (date) => {
            ...
            let text = `🌟 *Daily Report - ${global.formatDate(today)}*\n\n`;
            text += `📊 *Summary*\n`;
            text += `• Total Time: ${global.formatDuration(totalMins)}\n`;
            text += `• Study Time: ${global.formatDuration(studyMins)}\n`;
