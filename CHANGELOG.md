...
### Added
- **Modular Codebase**: Major refactoring to split the single `index.html` file into 18+ individual JavaScript modules for better maintainability and organization.
  - Components are now in `js/components/`.
  - Utilities are in `js/utils/`.
  - CSS is in `styles/main.css`.
  - Optimizer logic is in `js/optimizer/`.
- **IIFE Pattern**: All new JavaScript modules are wrapped in an Immediately Invoked Function Expression (IIFE) to avoid polluting the global namespace.

### Changed
- **`index.html` Cleanup**: The main `index.html` file has been reduced by over 96%, from 7,631 lines to 238 lines. It now primarily contains the HTML skeleton and script loading logic.
- **Development Workflow**: The development process is now more structured, with a clear separation of concerns between different parts of the application.

### Removed
- **Inline Scripts**: All inline JavaScript components and helper functions have been moved to external files.
- **Inline Styles**: All inline CSS has been moved to an external stylesheet.
